...
use_message = '''
BDVD Variance Decomposition.

Usage:
    bdvd-vd [options] <--ruv-dir ruv-dir> <design_file>
...
def launchTopLevelVD(fcdcPrx, facetAdminPrx):
    ...
    (rt,ofis)=fcdcPrx.GetFeatureObservers(rfi.SampleIDs)
    ks=[]
    extWs=[]
    featureIdxFrom =0
    featureIdxTo=0
    #check 
    if design.Ks is not None:
        ks=design.Ks
    
    if design.Ns is not None:
        extWs=design.Ns
    ...
    (rt,vds,amdTaskID)=ruvPrx.DecomposeVariance(
        ...
        gParams.num_threads,gParams.max_mem, gParams.out_file)
...
def main(argv=None):
    ...
    try:
        ...
        (rt, amdTaskID)=launchTopLevelVD(fcdcPrx,facetAdminPrx)
        preMsg=""
        amdTaskFinished=False
        bdvd_log("")
        bdvd_log("Variance Decomposition with {0} threads, {1} Mb RAM ...".format(gParams.num_threads,gParams.max_mem ))
        ...
        bdvd_log("Variance Decomposition [done]")
