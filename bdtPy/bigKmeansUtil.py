...
class singleNodeKmeansRunner:
    ...
    def prepare_kmeansc_config(self, kmeansc_tcp_port):
        ...
        replacements = {"__KMeansContractor_TCP_PORT__":str(kmeansc_tcp_port),
...
class kmeansContractorRunner:
    ...
    def prepare_kmeansc_config(self, kmeansc_tcp_port, kmeansc_worker_cnt):
       
        self.kmeansc_workercnt = kmeansc_worker_cnt
        self.kmeansc_ramsize = 2000

        infile = open("{0}/bdt/config/KMeansContractor.config".format(self.bdt_home_dir))
        outfile = open(self.kmeansc_dir+"/KMeansContractor.config", "w")
        replacements = {"__KMeansContractor_TCP_PORT__":str(kmeansc_tcp_port),
                        "__KMeansContractor_WorkerCnt__":str(self.kmeansc_workercnt),
                        "__KMeansContractor_RAMSIZE__":str(self.kmeansc_ramsize)}
        for line in infile:
            for src, target in replacements.items():
                line = line.replace(src, target)
            outfile.write(line)
