...
Usage:
    jabs-jointdata [options]  <design_file>
...
class JABSParams:
    ...
    def __init__(self):
        ...
        self.workflow_node = "jointsamples"
...
def attachJointSamples(fcdcPrx):
    ...
    import jabsJointSamplesDesign as design

    jointSamples=iBSDefines.JABSJointSamplesOutputDefine()
    jointSamples.SampleNames = design.SampleNames
    jointSamples.SingleResults = design.SingleResults
...
def makeBigMat(fcdcPrx, computePrx, jointSamples):
    #combine into big matrix
    task = computePrx.GetBlankVectors2MatrixTask()
    task.InOIDs = jointSamples.AttachedSingleOIDs
    task.FeatureIdxFrom = 0
    task.FeatureIdxTo = jointSamples.TotalWinCnt

    (rt, outOIDs,amdTaskID) = computePrx.Vectors2Matrix(task)
    ...
    if len(outOIDs)>0:
        ...
        (rt,bigmat_store_pathprefix)=fcdcPrx.GetFeatureValuePathPrefix(OIDs[0])
...
def main(argv=None):
    ...
    try:
        ...
        jointSamples = makeBigMat(fcdcPrx, computePrx, jointSamples)
        print(jointSamples.AttachedSingleOIDs)
        print(jointSamples.JointMat.ColNames)
        dumpOutput(jointSamples)
        
        bdvd_log("Running Joint Samples [done]")
