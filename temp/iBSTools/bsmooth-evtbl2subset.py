...
use_message = '''
Usage:
    bsmooth-evtbl2subset [options] -e <ev_bt2_sorted> <ref> <from> <to>
...
class Evtbl2SubsetParams:
    ...
    def parse_options(self, argv):
        try:
            opts, args = getopt.getopt(argv[1:], "hvo:e:",
                                        ["version",
                                         "help",
                                         "output-dir=",
                                         "ev-dir=",
        ...
        try:
            self.ref = args[0]
            self.pos_from = int(str(args[1]).replace(',',""))
            self.pos_to = int(str(args[2]).replace(',',""))
        except:
            raise Usage(use_message)
...
def processAllRefs():
    ...
    subset_path=output_dir+"{0}_{1}_{2}.tsv".format(gParams.ref, gParams.pos_from, gParams.pos_to)
    ...
    processOneRef(ref, evtblFilePath, subsetFile, gParams.pos_from, gParams.pos_to)
