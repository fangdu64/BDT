...
Usage:
    bdvd-ruv-tstats.py [options] <--ruv-dir ruv-dir> <--rowidxs rowidxs_file> <design-file>
...
def prepareNodeScripts():
    ...
    import bdvdRuvTstatsDesign as design
    ...
    replicateSamples = design.replicateSamples
    
    sampleIDs=[]
    for gpair in replicateSamples:
        for g in gpair:
            sampleIDs.extend(g)
...
def prepareScriptForR(replicateSamples, export_picke_file):
    #output group info
    group_info_fn="{0}groupinfo.txt".format(output_dir);
    outf = open(group_info_fn, "w")
    outf.write("GroupID\tg1ColIDFrom\tg1ColIDTo\tg2ColIDFrom\tg2ColIDTo\n")
    i=0
    colID=1
    for gpair in replicateSamples:
        sg1=gpair[0]
        sg2=gpair[1]
        i=i+1
        outf.write("{0}\t{1}\t{2}".format(i,colID,colID+len(sg1)-1))
        colID=colID+len(sg1)
        outf.write("\t{0}\t{1}\n".format(i,colID,colID+len(sg2)-1))
        colID=colID+len(sg2)
    outf.close()
...
def prepare_RScripts():
    ...
    # ==============================
    # RUV_Tstats.R
    infile = open("./iBS/iBS.R/BDVD/RUV_Tstats.R")
    outfile = open(RScriptDir+"/RUV_Tstats.R", "w")
...
def runTstats():
    ...
    r_cmd = [cmdpath,
                "--no-restore",
                "--no-save",
                "{0}/{1}".format(RScriptDir,"RUV_Tstats.R")]
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_logp()
        bdvd_log("Beginning RUV T-Stats run (v"+iBSUtil.get_version()+")")
        bdvd_logp("-----------------------------------------------")

        if not gParams.resume:
            replicateSamples = prepareNodeScripts()
            export_picke_file = exportBigMatrix()
            prepareScriptForR(replicateSamples,export_picke_file)
        prepare_RScripts()
        runTstats()
        bdvd_log("RUV T-Stats [done]")
