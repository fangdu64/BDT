...
Usage:
    bdvd-csv2mat [options] <design_file>
...
def uploadCSVData(fcdcPrx):
    ...
    bdvd_log("CSV data: rowCnt {0}, colCnt {1}".format(DomainSize,len(sampleNames)))
    (rt,fois)=fcdcPrx.GetFeatureObservers(observerIDs)
    featureValues = design.dataValues
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_logp()
        bdvd_log("Beginning BDVD CSV2Mat run (v"+iBSUtil.get_version()+")")
        bdvd_logp("-----------------------------------------------")
        ...
        (sampleNames,outOIDs, rowCnt)= uploadCSVData(fcdcPrx)
        ...
        for osi in osis:
            bdvd_logp("Sample {0}: Max = {1}, Min = {2}, Sum = {3}".format(osi.ObserverID, int(osi.Max), int(osi.Min), int(osi.Sum)))
