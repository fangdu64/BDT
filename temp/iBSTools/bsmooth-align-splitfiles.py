...
class BsAlignParalleParams:
    ...
    def __init__(self):
        self.start_splitIdx = 1
        self.num_splits = 20
        self.fastq_1_prefix = "/dcs01/gcode/fdu1/data/Kasper2014_WGBS/Quiescent1/SRR948844_1-split{0:02d}.fastq"
        self.fastq_2_prefix = "/dcs01/gcode/fdu1/data/Kasper2014_WGBS/Quiescent1/SRR948844_2-split{0:02d}.fastq"

        #self.fastq_1_prefix = "/dcs01/gcode/fdu1/data/Kasper2014_WGBS/Quiescent1/SRR948844_1-split{0}.fastq"
...
qsub_4strand_fr_text = '''
cd /home/student/fdu1/projects/Imprinting/buildHg19Index

mkdir $TMPDIR/bsmooth-fastq
cp __FASTQ1_FULLPATH__ $TMPDIR/bsmooth-fastq/1.fastq
cp __FASTQ2_FULLPATH__ $TMPDIR/bsmooth-fastq/2.fastq
mkdir $TMPDIR/bsmooth-genome
cp /dcs01/gcode/fdu1/data/Common/Homo_sapiens/UCSC/hg19/Sequence/WholeGenomeFasta/genome.fa $TMPDIR/bsmooth-genome/.

fastq_1=$TMPDIR/bsmooth-fastq/1.fastq
fastq_2=$TMPDIR/bsmooth-fastq/2.fastq

bowtie2Path="~/apps/build/bowtie2-2.2.3/bowtie2"
ref=$TMPDIR/bsmooth-genome/genome.fa
...
perl ${BSMOOTH_HOME}/bin/bswc_bowtie2_align.pl \\
		...
		--four-strand \\
		--no-unpaired \\
		--out=$TMPDIR/bsmooth-out/ev_bt2_raw \\
		--bscpg \\
		--bowtie2=${bowtie2Path} \\
		-- \\
		${index} \\
...
outDir="__SPLIT_OUTPUT__"
if [ ! -d "${outDir}" ]; then
        mkdir ${outDir}
fi

cp $TMPDIR/bsmooth-out/* ${outDir} -rfv
'''

qsub_shell_text_noTemp = '''
cd /home/student/fdu1/projects/Imprinting/buildHg19Index
