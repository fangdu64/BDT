...
use_message = '''
evaluate joint amr analysis

Usage:
    jabs-jamr-eval.py <--cpgmap cpgmap_pickle_file> <--samples jointsamples_pickle_file> <design_file>
...
def runJEval(fcdcPrx, amrPrx, jointsamples):
    ...
    import jamrEvalDesign as design
    
    (rt,task)=amrPrx.GetBlankJEvalKnownAMRTask()
    for amr in design.KnowAMRs:
        task.KnownAMRs.append(
             iBS.BpRange(amr[1], amr[0],int(amr[2])-1,int(amr[3])-1))
    ...
    task.TopCpGCnt=design.TopCpGCnt
    task.MergeDistance=design.MergeDistance
    print(task)

    (rt,taskID)=amrPrx.RunJEvalKnownAMR(task)
    return (taskID,task)
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_log("")
        bdvd_log("Running JEval Known AMRs with {0} threads, {1} Mb RAM ...".format(gParams.num_threads,gParams.max_mem ))
        ...
        bdvd_log("Running JEval Known AMRs [done]")
