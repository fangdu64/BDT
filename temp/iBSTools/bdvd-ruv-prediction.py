...
Usage:
    bdvd-ruv-prediction.py [options] <design-file>
...
class BDVDParams:
    ...
    def __init__(self):
        ...
        self.KsX=None
        self.NsX=None
        self.colIDs_train = None
        self.colIDs_test = None
        self.min_predictor_cnt=20
        self.MatDirs=None
...
def exportPairedMats():
    ...
    import ruvPredictDesign as design
    pairedMatNames=[
            "MatX",
            "MatY"]
    pairedMatDesigns=[
    ...
    gParams.colIDs_train = design.ColIDs_Train
    gParams.colIDs_test = design.ColIDs_Test
    gParams.min_predictor_cnt=design.MinPredictorCnt
...
def prepare_RScripts():
    ...
    # ==============================
    # RUV_Fstats.R
    infile = open("./iBS/iBS.R/BDVD/RUV_Prediction.R")
    outfile = open(RScriptDir+"/RUV_Prediction.R", "w")
    replacements = {"__RSCRIPT_DIR__":RScriptDir,
                    ...
                    "__MIN_PREDICTOR_CNT__":str(gParams.min_predictor_cnt),
                    "__ColIDs_Train__":str(gParams.colIDs_train).replace('[','').replace(']',''),
                    "__ColIDs_Test__":str(gParams.colIDs_test).replace('[','').replace(']','')
                    }
    for line in infile:
        for src, target in replacements.items():
            line = line.replace(src, target)
...
def runPrediction():
    ...
    r_cmd = [cmdpath,
                "--no-restore",
                "--no-save",
                "{0}/{1}".format(RScriptDir,"RUV_Prediction.R")]
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_logp()
        bdvd_log("Beginning RUV Prediction run (v"+iBSUtil.get_version()+")")
        bdvd_logp("-----------------------------------------------")

        exportPairedMats()
        prepare_RScripts()
        runPrediction()
        bdvd_log("RUV Prediction [done]")
