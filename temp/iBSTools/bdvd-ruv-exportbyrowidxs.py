...
Usage:
    bdvd-ruv-exportbyrowidxs [options] <--ruv-dir ruv-dir> <--rowidxs rowidxs_file> <design-file>
...
output_dir = "./exportbyrowidxs_out/"
...
class BDVDParams:
    ...
    def __init__(self):
        ...
        self.workflow_node = "ruv-exportbyrowidxs"
...
def launchExportTask(fcdcPrx, facetAdminPrx, computePrx):
    ...
    import bdvdRuvExportByRowIdxsDesign as design
    ...
    # bdvd_log("ColIDs: {0}".format(sampleIDs))
    (rt,ofis)=fcdcPrx.GetFeatureObservers(sampleIDs)
    outpath=os.path.abspath(output_dir)
    tasks=[]
    bfvFiles=[] # assume each matrix outputed in one file
    for i in range(len(ks)):
        task=computePrx.GetBlankExportByRowIdxsTask()
        task.TaskName="k={0},n={1}".format(ks[i],extWs[i])
        task.reader=ruvPrx
        task.SampleIDs= sampleIDs
        task.FeatureIdxs=featureIdxs
        task.OutID=10001+i
        task.OutPath=outpath
        bfvFiles.append("{0}/gid_{1}.bfv".format(task.OutPath,task.OutID))
        tasks.append(task)
    ...
    bdvd_log("rt {0}, amdTaskID {1}".format(rt,amdTaskID))
