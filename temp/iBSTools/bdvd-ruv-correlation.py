...
use_message = '''
BDVD Evaluate whether RUV improves the signal noise ratio of the correlated pairs

Usage:
    bdvd-ruv-correlation.py [options] <design-file>
...
class BDVDParams:
    ...
    def __init__(self):
        ...
        self.workflow_node = "ruv-correlation"
...
def exportPairedMats():
    ...
    import ruvCorrDesign as design
    pairedMatNames=[
            "SignalMate1",
            "SignalMate2",
            "NoiseMate1",
            "NoiseMate2"]
    pairedMatDesigns=[
            design.SignalMate1Design,
            design.SignalMate2Design,
            design.NoiseMate1Design,
            design.NoiseMate2Design]
    matDirs=[]
    ...
    gParams.KsMate1=design.KsMate1
    gParams.NsMate1=design.NsMate1
    gParams.KsMate2=design.KsMate2
    gParams.NsMate2=design.NsMate2
...
def prepare_RScripts():
    ...
    # ==============================
    # RUV_Fstats.R
    infile = open("./iBS/iBS.R/BDVD/RUV_Correlation.R")
    outfile = open(RScriptDir+"/RUV_Correlation.R", "w")
    replacements = {"__RSCRIPT_DIR__":RScriptDir,
                    "__SignalMate1__":gParams.MatDirs[0],
                    "__SignalMate2__":gParams.MatDirs[1],
                    "__NoiseMate1__":gParams.MatDirs[2],
                    "__NoiseMate2__":gParams.MatDirs[3],
                     "__KsMate1__":str(gParams.KsMate1).replace('[','').replace(']',''),
                    "__NsMate1__":str(gParams.NsMate1).replace('[','').replace(']',''),
                    "__KsMate2__":str(gParams.KsMate2).replace('[','').replace(']',''),
                    "__NsMate2__":str(gParams.NsMate2).replace('[','').replace(']',''),
                    "__OUT_DIR__":output_dir[:-1],
                    "__NUM_THREADS__":str(gParams.num_threads),
                    "__OnewayConfig__":strOnewayConfig,
                    "__H1H0Ratio__":str(gParams.H1H0Ratio),
                    "__MAX_FDR__":str(gParams.MAX_FDR)
                    }
    for line in infile:
        for src, target in replacements.items():
            line = line.replace(src, target)
...
def runCorrelation():
    ...
    r_cmd = [cmdpath,
                "--no-restore",
                "--no-save",
                "{0}/{1}".format(RScriptDir,"RUV_Correlation.R")]
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_logp()
        bdvd_log("Beginning RUV Correlation run (v"+iBSUtil.get_version()+")")
        bdvd_logp("-----------------------------------------------")

        exportPairedMats()
        prepare_RScripts()
        runCorrelation()
        bdvd_log("RUV Correlation [done]")
