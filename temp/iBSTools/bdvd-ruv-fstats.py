...
Usage:
    bdvd-ruv-fstats.py [options] <--ruv-dir ruv-dir> <--rowidxs rowidxs_file> <design-file>
...
def prepareNodeScripts():
    ...
    import bdvdRuvFstatsDesign as design
    ...
    commonSamples = design.commonSamples
...
def prepare_RScripts():
    ...
    # ==============================
    # RUV_Fstats.R
    infile = open("./iBS/iBS.R/BDVD/RUV_Fstats.R")
    outfile = open(RScriptDir+"/RUV_Fstats.R", "w")
...
def runFstats():
    ...
    r_cmd = [cmdpath,
                "--no-restore",
                "--no-save",
                "{0}/{1}".format(RScriptDir,"RUV_Fstats.R")]
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_logp()
        bdvd_log("Beginning RUV F-Stats run (v"+iBSUtil.get_version()+")")
        bdvd_logp("-----------------------------------------------")
