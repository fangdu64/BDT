...
use_message = '''
BDVD Calculate Quantiles for a list of samples.

Usage:
    bdvd-quantile.py [options] <--ruv-dir ruv-dir> <design-file>
...
def runQuantileTask(fcdcPrx, computePrx):
    # configuration by user
    designPath=os.path.abspath(script_dir)
    if designPath not in sys.path:
         sys.path.append(designPath)
    import bdvdQuantileDesign as design

    task=computePrx.GetBlankQuantileTask()
    task.reader = fcdcPrx
    task.SampleIDs = design.SampleIDs
    if design.FeatureIdxFrom is not None:
        task.FeatureIdxFrom = design.FeatureIdxFrom
        task.FeatureIdxTo = design.FeatureIdxTo
    if design.Quantiles is not None:
        task.Quantiles = design.Quantiles

    (rt, qFeatureIdxs, qValues) = computePrx.Quantile(task)
    bdvd_log(" Quantiles: {0}".format(task.Quantiles))
    bdvd_log(" qValues: {0}".format(qValues))
    bdvd_log(" qFeatureIdxs: {0}".format(qFeatureIdxs))
    task.reader=None
    hvfOut=iBSDefines.QuantileOutputDefine(task.Quantiles,qFeatureIdxs,qValues,task)
    fn = "{0}{1}".format(output_dir,gParams.result_dumpfile)
    iBSDefines.dumpPickle(hvfOut,fn)
    bdvd_log(" outputed at ".format(os.path.abspath(fn)))
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_logp()
        bdvd_log("Beginning bdvd-quantile (v"+iBSUtil.get_version()+")")
        bdvd_logp("-----------------------------------------------")
        ...
        runQuantileTask(fcdcPrx, computePrx)
        
        bdvd_log("bdvd-quantile [done]")
