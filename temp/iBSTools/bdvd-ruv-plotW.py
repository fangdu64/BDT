...
def exportWt(facetAdminPrx, k, extW):
    ...
    Wt.fromlist(values)
    Wt_fn="{0}Wt.bfv".format(output_dir)
    outf = open(Wt_fn, "wb")
    Wt.tofile(outf)
    outf.close()
...
def prepareRScripts():
    ...
    # ==============================
    # RUV_Fstats.R
    infile = open("./iBS/iBS.R/BDVD/RUV_PlotW.R")
    outfile = open(RScriptDir+"/RUV_PlotW.R", "w")
...
def runRScript():
    ...
    r_cmd = [cmdpath,
                "--no-restore",
                "--no-save",
                "{0}/{1}".format(RScriptDir,"RUV_PlotW.R")]
...
def main(argv=None):
    ...
    try:
        ...
        bdvd_logp()
        bdvd_log("Beginning RUV plotW run (v"+iBSUtil.get_version()+")")
        bdvd_logp("-----------------------------------------------")
