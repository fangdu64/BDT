...
use_message = '''
KMeans++ prepare initial seeds (multihreads, local node)
...
def prepare_output_dir():
    ...
    shutil.copy(gParams.design_file,"{0}/bigclustPPSeedsDesign.py".format(script_dir))
...
def prepare_kmeansc_config(fcdc_tcp_port,kmeansc_tcp_port):
    ...
    import bigclustPPSeedsDesign as design
...
def prepareKMeansProject(facetAdminPrx, fcdcPrx, kmeansSAdminPrx, dataOIDs, dataMat):
    designPath=os.path.abspath(script_dir)
    if designPath not in sys.path:
         sys.path.append(designPath)
    import bigclustPPSeedsDesign as design
    ...
    rqstProj.ProjectName = "KMeans++ Seeds"
    rqstProj.Task = iBS.KMeansTaskEnum.KMeansTaskPPSeeds
    rqstProj.K=design.K
    rqstProj.Distance = design.Distance
    rqstProj.MaxIteration = design.K
    rqstProj.MinChangeCnt = 0
...
def saveKMeansResult(fcdcPrx, kmeansServerPrx,proj, dataMat):
    ...
    outObj.BigMat = bigmat
    bdvd_log("save cluster centroids, {0} rows {1} cols".format(K,dataColCnt))

    #
    # Save seeds feature idxs
    #
    (rt,values)=kmeansServerPrx.GetPPSeedFeatureIdxs(projectID)
    outObj.SeedFeatureIdxs=values
    bdvd_log(str(values))
    bdvd_log("save cluster featureidxs")
...
def main(argv=None):
    ...
    try:
        ...
        rqstProj = prepareKMeansProject(facetAdminPrx, fcdcPrx, kmeansSAdminPrx, dataOIDs, dataMat)
        ...
        bdvd_log("")
        bdvd_log("Running KMeans++ seeds with {0} threads, {1} Mb RAM ...".format(gParams.kmeansc_workercnt,gParams.kmeansc_ramsize))
        ...
        # Output Results
        # -----------------------------------------------------------
        saveKMeansResult(fcdcPrx, kmeansServerPrx,retProj, dataMat)
