...
for(i in 1:nrow(AMRs))
{
	...
	for(w in 1:slidingWinCnt)
	{
		...
		siteMUCnts=calcSiteMUCnt(evTbl)
		minMUCntPerSite =4
		minEvCntPerRead =2
		readID2EvRowIDs = constructReadsWithEvidence(evTbl,siteMUCnts, minMUCntPerSite, minEvCntPerRead)
		if(length(readID2EvRowIDs)<uniqe_readcnt_min)
			next
		sites= calcValidSites(readID2EvRowIDs,evTbl )

		high_prob = 0.75
		low_prob = 0.25
		max_itr =100
		p = test_asm_lrt(max_itr, low_prob, high_prob, sites, readID2EvRowIDs, evTbl)
		...
		lineInfo = paste(w+w0, format(ret$outter_posFrom,scientific=FALSE), 
			...
			CpGSiteCnt, ReadCnt, WindowSize, p$em_iter, p$p_value, p$llr_stat, p$df,1-p$p_value, sep="\t")
		cat(lineInfo,"\n", file =outfile)
